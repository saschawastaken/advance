...
class Player():
    ...
    def move(self, win, ws, mousePos, size):
        # PLAYER ROTATING
        mx, my = mousePos
        ...
        if self.BuildMode:
            self.build(ws, size, mousePos)
        
        else:
            self.shoot(ws, mousePos, size)
    def shoot(self, ws, mousePos, size):
    ...
    def build(self, ws, size, mousePos):
        if self.build_material > 0:
            if self.mButtonDown:
                rows = ws[0] / size
